...
from idm.models.dsbn import convert_dsbn, convert_bn
from idm.models.idm_dsbn import convert_dsbn_idm, convert_bn_idm
from idm.evaluators import Evaluator
...
def main_worker(args):
    ...
    if args.dsbn_idm:
        print("==> Load the model with domain-specific BNs (IDM)")
    ...
    if args.dsbn_idm:
        print("==> Test with {}-domain BNs".format("source" if args.test_source else "target"))
        convert_bn_idm(model, use_target=(not args.test_source))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dsbn-idm', action='store_true',
                        help="test on the model with domain-specific BN (IDM)")
